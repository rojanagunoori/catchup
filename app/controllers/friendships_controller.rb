...
class FriendshipsController < ApplicationController
  def create
    friend = User.find(params[:friend_id])
    if current_user.friends.include?(friend) || current_user.pending_friend_requests.exists?(friend_id: friend.id)
      flash[:alert] = "Unable to send friend request: Friend request already sent."
    else
      @friendship = current_user.sent_friend_requests.build(friend: friend, status: :pending)
      flash[:notice] = @friendship.save ? "Friend request sent!" : "Unable to send friend request."
    end
    redirect_to friends_path
  end
